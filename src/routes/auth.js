...
router.post('/bootstrap-superadmin', requireAuth(['admin']), asyncHandler(async (req, res) => {
...
router.get('/usuarios', requireAuth(['super_admin']), asyncHandler(async (req, res) => {
  const usuarios = await prisma.usuario.findMany({ select: { id: true, correo: true, rol: true, creadoEn: true }, orderBy: { correo: 'asc' } });
  res.json(usuarios);
}));

router.post('/usuarios', requireAuth(['super_admin']), asyncHandler(async (req, res) => {
...
router.delete('/usuarios/:id', requireAuth(['super_admin']), asyncHandler(async (req, res) => {
  try {
    await prisma.usuario.delete({ where: { id: req.params.id } });
    res.json({ ok: true });
  } catch (e) {
    if (e.code === 'P2025') return res.status(404).json({ error: 'Ese usuario ya no existe (puede que ya se haya eliminado).' });
    throw e;
  }
}));

// El admin resetea la contraseña de cualquier usuario (ej. si a alguien se le olvidó).
router.put('/usuarios/:id/password', requireAuth(['super_admin']), asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (e.code === 'P2025') return res.status(404).json({ error: 'Ese usuario no existe.' });
    throw e;
  }
}));

// Cambiar el rol de un usuario existente (solo super_admin).
router.put('/usuarios/:id/rol', requireAuth(['super_admin']), asyncHandler(async (req, res) => {
  const { rol } = req.body;
  if (!['admin', 'lectura', 'super_admin'].includes(rol)) return res.status(400).json({ error: 'rol debe ser "admin", "lectura" o "super_admin"' });
  try {
...
router.post('/cambiar-password', requireAuth(['admin','lectura','super_admin']), asyncHandler(async (req, res) => {
